...
$pendingCount = $pdo->query("SELECT COUNT(*) FROM telegram_orders WHERE status='pending'")->fetchColumn();
$approvedTotal = $pdo->query("SELECT SUM(amount) FROM telegram_orders WHERE status='approved'")->fetchColumn() ?: 0;
$rejectedCount = $pdo->query("SELECT COUNT(*) FROM telegram_orders WHERE status='rejected'")->fetchColumn();

$filter = $_GET['filter'] ?? 'pending';
$where = in_array($filter, ['pending', 'approved', 'rejected'], true) ? "WHERE o.status='$filter'" : '';
